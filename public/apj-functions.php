...
class PreventLandscapeRotation
{
    ...
    public static function APJPreventLandscapeRotationPublish()
    {
       ...
       wp_add_inline_script( 'prevent-landscape-rotation', 'function isLandscape() {return (window.orientation === 90 || window.orientation === -90);}jQuery(document).ready(function() {APJorientationChangeFnc();});jQuery(window).on(\'orientationchange\', function(event) {if (isLandscape()) {APJorientationChangeFnc();}else{swal.close();}});function APJorientationChangeFnc() {if (isLandscape()) {const wrapper = document.createElement(\'div\'); wrapper.innerHTML = "<img class=\'swal2-image apjnolandscapeimage\' src=\''.APJ_PLR_PLUGIN_IMAGES_PATH.'landscape-pic.png\' style=\'width: 30%;margin-left: auto;margin-right: auto;height: 84px;margin-bottom: 8px;display: block;\'><p style=\'color:' . (empty($plr_txt_clr) ? '#000000' : $plr_txt_clr) . '\'><br>' . (empty($plr_message_show) ? $apj_plr_default_message : $plr_message_show) . '</p>";swal({content: wrapper,allowOutsideClick: false,closeOnClickOutside: false,closeOnEsc: false,});jQuery(".swal-button--confirm").css("display","none");jQuery(\'.swal-overlay\').css(\'background-color\', \'' . (empty($plr_bg_clr) ? 'rgba(216, 216, 216, 0.94)' : $plr_bg_clr) . '\');jQuery(\'.swal-modal\').css(\'background-color\', \'rgba(0, 174, 83, 0)\');}else{swal({timer: 2,className: "apjtempswal",});swal.close();}}' );
     }
    /**
    ...
    {
        echo "<style>.apjtempswal{display:none!important;}body.complete .swal-overlay{opacity: 0;}body.complete .swal-overlay--show-modal{opacity: 1 !important;}</style>";
    }
    /**
